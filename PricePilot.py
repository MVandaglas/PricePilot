...
if st.session_state.offer_df is not None:
    ...
    if not edited_df.equals(st.session_state.offer_df):
        eedited_df["M2 totaal"] = edited_df.apply(
    lambda row: float(row["Aantal"]) * float(re.sub(r'[^0-9.]', '', row["M2 p/s"]))
    if pd.notna(row["Aantal"]) and pd.notna(row["M2 p/s"]) and re.sub(r'[^0-9.]', '', row["M2 p/s"]).strip() != ''
    else None,
    axis=1
)
