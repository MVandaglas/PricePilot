...
def extract_data_with_gpt(prompt):
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "Je bent een geavanceerde extractietool die glassamenstellingen extraheert uit een bestekformulier en vertaalt naar een JSON-tabel. Kap niet af, verwerk het gehele document."},
                {"role": "user", "content": prompt}
            ]
        )
        ...
        if not attachment_name.lower().endswith(excluded_extensions):
            if st.button(
                ...
                with st.spinner(f"HawkAI-extractie bezig voor {attachment_name}... ⏳"):
                    extracted_text = extract_data_with_gpt(attachment)  # Jouw AI-extractiefunctie
                    st.write("📄 Geëxtraheerde tekst:")
